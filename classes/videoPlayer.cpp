...
VideoPlayer::VideoPlayer(const std::string &videoFile, const std::string &detectorFile) {
    try{
        ...
        frame_width = videoCapture.get(CV_CAP_PROP_FRAME_WIDTH);
        frame_height = videoCapture.get(CV_CAP_PROP_FRAME_HEIGHT);
...
void VideoPlayer::save(std::string &outName) {
    ...
    cv::VideoWriter videoWriter(outName, CV_FOURCC('M', 'J', 'P', 'G'), fps, cv::Size(frame_width, frame_height));
